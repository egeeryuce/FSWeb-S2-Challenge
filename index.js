...
function cumlelereDonustur(cumleler, ayrac = ",") {
    ...
    let sonuc= cumleler.map(a => a.join(ayrac))
    return sonuc
}
...
function paragrafOlustur(cumleler, cumleKur, cumlelereDonustur) {
  let newArray=cumlelereDonustur(cumleler," ")
  let paragraf=cumleKur(newArray[1],newArray[3],newArray[5],newArray[7],newArray[9])
  return paragraf
}
...
Kirpiyi dizinin son elemanına ekleyin (.push ve .unshift metotlarını kullanın)
...
function emojileriDonustur(masaj, emojiler) {
  let keys=Object.keys(emojiler)
  for(let i=0;i<keys.length;i++){
    masaj=masaj.replaceAll(keys[i].toUpperCase(),emojiler[keys[i]])
    masaj=masaj.replaceAll(keys[i].toLowerCase(),emojiler[keys[i]])
  }
  return masaj
}
